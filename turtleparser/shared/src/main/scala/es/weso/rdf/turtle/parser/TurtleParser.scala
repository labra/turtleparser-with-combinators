...
*/
  ...
  def triples(s: TurtleParserState): Parser[(List[RDFTriple], TurtleParserState)] =
  ...
  def subjPredicatesObjectList(s: TurtleParserState): Parser[((RDFNode, List[(IRI, List[RDFNode])]), TurtleParserState)] = {
    seqState(subject, predicateObjectList)(s) ^^ {
      case (n ~ ls, s) => ((n, ls), s)
    }
  }

  def predicateObjectList(s: TurtleParserState): Parser[(List[(IRI, List[RDFNode])], TurtleParserState)] =
    rep1sepOptState(s, verbObjectList, token(";"))

  def verbObjectList(s: TurtleParserState): Parser[((IRI, List[RDFNode]), TurtleParserState)] =
    opt(WS) ~> verb(s.namespaces) ~ objectList(s) ^^ {
      case node ~ objs => ((node, objs._1), objs._2)
    }

  def objectList(s: TurtleParserState): Parser[(List[RDFNode], TurtleParserState)] =
  ...
  def rdf_object(s: TurtleParserState): Parser[(RDFNode, TurtleParserState)] =
    opt(WS) ~>
      (iri(s.namespaces) ^^ { case iri => (iri, s) }
